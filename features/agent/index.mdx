...
flowchart TD
    ...
    D --> E[Dependency-aware node planning]
    E --> F[Node preflight and tool filtering]
    F --> G[Retry-aware graph executor]
    G --> H[Node observations plus risk framing]
    H --> I[Response composer synthesis]
    I --> J[TradingAgent runtime execution]
    J --> K[Assistant text]
    J --> L[Structured UI events]
    J --> M[Execution-aware guidance]
    J --> N[Pipeline trace and fallback metadata]
    G --> M[Session artifact store]
```

## First specialist-like nodes: chart analysis, market snapshot, research snapshot, portfolio snapshot, execution snapshot, memory snapshot, risk review, general fallback, clarification prep, and response composer

The first real nodes in the composable pipeline are `chart_analysis`, `market_snapshot`, `research_snapshot`, `portfolio_snapshot`, `execution_snapshot`, `memory_snapshot`, `risk_review`, `general_fallback`, `clarification_prep`, and `response_composer`.
...
The planner is now dependency-aware too.

That means the graph no longer plans or executes every node just because the request mentions a broad tool group.

Examples:

| Planner rule | What it changes |
| --- | --- |
| no symbol can be resolved before a market snapshot | `market_snapshot` is not planned, or is skipped before tool calls |
| no upstream observations exist for risk framing | `risk_review` is skipped instead of pretending it has context |
| low-confidence route | the graph stays on `general_fallback -> response_composer` even if the raw text mentions charts or indicators |

Nodes also now have bounded retry/backoff behavior where it helps:

| Node family | Retry policy |
| --- | --- |
| data-fetching nodes such as `chart_analysis`, `market_snapshot`, `research_snapshot`, `portfolio_snapshot`, `execution_snapshot`, and `memory_snapshot` | can retry once with a short bounded backoff when the result looks transient |
| semantic/control nodes such as `risk_review`, `general_fallback`, `clarification_prep`, and `response_composer` | do not retry because they are deterministic by design |

That keeps the graph more resilient without turning it into an unbounded autonomous loop.
...
`risk_review` adds a different kind of value. It does not fetch new market data. Instead, it inspects the observations already gathered by earlier nodes and turns them into explicit downside framing:

| It does | It avoids |
| --- | --- |
| check whether explicit invalidation or stop markers are present | pretending to know risk controls that were never observed |
| flag fresh-news sensitivity and upstream degraded context | fetching another broad research round just to restate caution |
| turn indicator extremes such as RSI overbought or oversold into risk flags | replacing the final model with a second free-form analyst |
| inject a compact caution summary before the final answer | mutating execution or chart state |

That framing is now richer than a plain caution flag. The node now also derives:

- `invalidation_quality`
- `confluence_strength`
- `news_fragility`
- `execution_readiness`
- a normalized `risk_summary`
...
`response_composer` is no longer just a passthrough summary. It now adds:

| Composition output | Why it matters |
| --- | --- |
| `primary_evidence_nodes` | tells the final runtime which node outputs deserve the most weight |
| `supporting_nodes` | keeps secondary context visible without over-weighting it |
| `conflict_flags` | makes disagreements or fragility explicit instead of burying them |
| `recommended_posture` | nudges the final answer toward `direct`, `balanced`, `cautious`, `risk_first`, or fallback behavior |
| resolved symbol/timeframe | keeps the final answer anchored to the actual asset context |
| artifact count | lets the runtime know when chart-write artifacts exist for this turn |
...
flowchart LR
    ...
    G --> H[Persist chart artifact]
    H --> F
    F --> I[risk_review when planned]
    I --> J[response_composer synthesizes posture and conflicts]
``` 
...
That write path now also persists a session-scoped artifact when a `scope_id` exists.

So a successful chart-write turn can now leave behind:

- the chart mutation summary
- the resulting screenshot URL
- the node and scope that produced it

This makes chart-write turns easier to reopen, inspect, or reference later from the frontend or from tooling.
