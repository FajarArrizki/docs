...
The chart-analysis node still has the strictest boundaries:

| Allowed | Blocked |
| --- | --- |
| read chart state | chart drawings |
| change symbol when scope is global | chart annotations and write workflows |
| keep symbol locked when scope is `locked_asset` | alert creation as part of analysis |
| set timeframe | permanent chart-writing behavior |
| add indicators temporarily | unrestricted chart mutation |
| read indicator values and quote data | |

Internally it follows a bounded `Reason -> Act -> Critique -> Observe` loop so the node can prepare chart evidence before the final response without drifting into uncontrolled tool spam.
...
That low-confidence path is also planner-gated now. A raw mention of charts, RSI, or MACD no longer forces `chart_analysis` to run when routing confidence is low.
