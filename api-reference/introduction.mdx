...
- memory, model-management, and service-cost utilities
...
<<<<<<< HEAD
- inspect model metadata and service-cost state
=======
- inspect model and service-cost state

The agent surface also now carries more structured backend reasoning than before:

- bounded node execution before the final answer
- explicit risk framing
- explicit response posture guidance
- persisted chart-write artifacts per `scope_id`
...
<<<<<<< HEAD
| Models | OpenRouter model catalog, provider metadata, and model-side pricing hints |
| Service Cost | combined model cost, monitoring/service cost, and settlement-ready summaries |
| Streaming | WebSocket price stream |
=======
| News | asset-specific news snapshot plus live news WebSocket feed |
| Models | OpenRouter model catalog surfaces |
| Service Cost | combined model and monitoring cost tracking |
| Streaming | SSE agent output plus WebSocket price and news transport |

## What changed in the current backend generation

The current generation of the backend API is more pipeline-aware than earlier versions.

That means:

- the agent entrypoint is still stable for clients
- but the runtime behind it now uses dependency-aware node planning
- risk review is now a first-class pre-response step
- final answer composition now includes conflict detection and recommended posture

So the API contract stayed simple while the runtime behavior got more structured.
>>>>>>> b00c72694c26ea1bb339b9487e48b046d7802d53
...
5. storage-backed support services such as memory, model metadata, and cost ledgers
...
4. Assets
<<<<<<< HEAD
5. Models if you need OpenRouter model metadata
6. Service Cost if you need combined billing or settlement summaries
=======
5. Models if you need model catalog access
6. Service Cost if you need pay-as-you-go cost tracking
>>>>>>> b00c72694c26ea1bb339b9487e48b046d7802d53
...
<<<<<<< HEAD
    OpenRouter model catalog, provider metadata, and pricing-facing model data.
  </Card>
  <Card title="Service Cost" icon="receipt" href="/api-reference/service-cost">
    Combined model and monitoring cost summaries for settlement-aware product flows.
=======
    OpenRouter model catalog and capability metadata.
  </Card>
  <Card title="Service Cost" icon="wallet" href="/api-reference/agent">
    Combined model and monitoring cost summaries for one scope.
>>>>>>> b00c72694c26ea1bb339b9487e48b046d7802d53
  </Card>
