...
| Program id | `DLm4eybf935a2TUgiBTvENmZMCMV9fa63wvVP3n7HW21` |
...
| --- | --- | --- |
| Program / contract | `DLm4eybf935a2TUgiBTvENmZMCMV9fa63wvVP3n7HW21` | current devnet deployment |
...
| PlatformConfig PDA | `F9buv5ScBe1iYHfZgv3NSGygSSUCzGmt1fjnHdp1Pbdz` | singleton policy account |
| FeeVault PDA | `FvN3wwxDm4oE7caFdHdBXmgHxNd8fSPcAgJmVcdnRURq` | singleton protocol fee account |

### Dynamic addresses by design

| Component | Derivation | Why it is not one fixed address |
| --- | --- | --- |
| `Vault` | `PDA(["vault", owner])` | one vault per user |
| `DelegatedSigner` | `PDA(["delegated_signer", owner, delegate])` | one account per delegation pair |
| `AiUsageRecord` | `PDA(["ai_usage", vault, usage_sequence])` | one immutable receipt per charge |
...
#### 2.4 Long Model Id Handling
- Some OpenRouter model ids are longer than Solana's 32-byte PDA seed limit.
- For those cases, the backend seeds the registry with a deterministic shortened on-chain alias.
- The source id and on-chain registry id mapping is exported to `target/model-registry-aliases.json` for backend lookup.
