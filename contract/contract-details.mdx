...
- **Effect**: Blocks record_ai_usage calls
...
- **Process**:
  1. Calculate total cost (base + markup + platform fee)
  2. Check vault balance
  3. Transfer platform fee to fee_vault
  4. Update vault balance
  5. Create immutable usage record
  6. Update model stats (if registry provided)
- **Authority**: User (vault owner)
...
- Base cost: 1000 lamports (backend pays provider off-chain)
- Markup: 50 lamports (backend's revenue)
- Platform fee: 52 lamports (extracted to fee_vault)
