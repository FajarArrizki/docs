---
title: "Rabit Contract"
description: "The Solana program layer for pay-as-you-go charging, delegated backend authority, user vaults, and on-chain usage accounting."
sidebarTitle: "Overview"
---

Rabit Contract is the Solana program that lets the product charge for backend work in a wallet-native way.

Instead of keeping pricing and authorization fully off-chain, it introduces a contract-side payment and permission layer built around vaults, delegated signers, usage records, and fee accounting.

## Contract Address

| Field | Value |
| --- | --- |
| Contract name | `rabit_contract` |
| Devnet program address | `DLm4eybf935a2TUgiBTvENmZMCMV9fa63wvVP3n7HW21` |

## Devnet Address Breakdown

### Singleton addresses

These are the fixed addresses for the current devnet deployment.

| Component | Address | Notes |
| --- | --- | --- |
| Program / contract address | `DLm4eybf935a2TUgiBTvENmZMCMV9fa63wvVP3n7HW21` | main program id |
| Admin authority | `HfMHdkJuHztBm1y2JpPfZwauqesZQdmenQaHVL3J9wMP` | current config authority |
| Backend authority | `HfMHdkJuHztBm1y2JpPfZwauqesZQdmenQaHVL3J9wMP` | current backend signer for delegated usage |
| Platform config PDA | `F9buv5ScBe1iYHfZgv3NSGygSSUCzGmt1fjnHdp1Pbdz` | singleton config account |
| Fee vault PDA | `FvN3wwxDm4oE7caFdHdBXmgHxNd8fSPcAgJmVcdnRURq` | protocol fee collection account |

### Dynamic addresses

These addresses are derived per user or per model, so there is no single permanent address for the whole contract.

| Component | Address pattern | Why it is dynamic |
| --- | --- | --- |
| User vault | `PDA(["vault", owner])` | every user gets their own prepaid vault |
| Delegated signer | `PDA(["delegated_signer", owner, delegate])` | every owner/delegate pair gets its own delegation account |
| Usage record | `PDA(["ai_usage", vault, usage_sequence])` | every usage charge creates a new immutable receipt |
| Model registry | `PDA(["model_registry", model_id])` | every model has its own registry account |

For the full current model registry list, see [/contract/model-registry-catalog](/contract/model-registry-catalog).

## Why it matters

Rabit Contract gives the product a simple charging model with real operational weight:

- the user preloads SOL into a vault
- the user delegates limited charging authority to the backend
- the backend records one settlement-ready charge
- the contract enforces the debit and stores an immutable receipt

That makes the contract more than a payment gimmick. It shows how backend AI work and monitoring work can become a user-auditable on-chain charge path.

## What it manages

| Domain | Purpose |
| --- | --- |
| user vaults | hold prepaid SOL for AI usage |
| delegated signers | allow bounded backend authority with expiry and spending limits |
| usage records | create an immutable on-chain log of each recorded charge |
| platform config | stores admin authority, backend authority, fee settings, and pause state |
| fee vault | accumulates platform fees |
| model registry | stores optional model catalog metadata and pricing hints |

## Why it exists

| Problem | Contract-side answer |
| --- | --- |
| model and monitoring billing are usually opaque and off-chain | backend cost can be charged against an on-chain vault |
| backend automation often needs dangerous long-lived authority | delegated signers let the user scope time and spend limits |
| fee and markup accounting is hard to audit | fee flow becomes part of program state |
| Solana apps keep rebuilding the same payment pattern | the contract turns it into a reusable primitive |

## Core flow

```mermaid
sequenceDiagram
    participant User
    participant Backend
    participant Contract

    User->>Contract: initialize_vault
    User->>Contract: deposit_to_vault
    User->>Contract: create_delegated_signer
    Backend->>Contract: record_ai_usage_with_delegation(base_cost, service_cost, ...)
    Contract-->>Backend: debit vault, credit fee vault, emit usage record
```

## Key Product Value

| Theme | Rabit's answer |
| --- | --- |
| wallet-native trust | users fund a dedicated prepaid vault |
| backend automation without unlimited trust | delegated signer uses expiry and spending caps |
| transparent charging | every accepted charge creates an immutable usage record |
| extensible billing model | the contract supports both model cost and service cost |

## Read this next

| If you want... | Read |
| --- | --- |
| the real build and test workflow | [/getting-started/run/quickstart-contract](/getting-started/run/quickstart-contract) |
| architecture and account model | [/contract/architecture](/contract/architecture) |
| how model cost and monitoring cost become one settlement call | [/contract/service-cost](/contract/service-cost) |
| instruction-level reference | [/contract/api-reference](/contract/api-reference) |
| security analysis | [/contract/security](/contract/security) |
