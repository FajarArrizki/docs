...
When OpenRouter is enabled, the backend can accumulate estimated model usage cost per chat/session scope.
...
4. the latest accumulated summary is returned in:
   ...
   - `POST /api/agent/chat` as part of combined `service_cost`
...
- monitoring cost is tracked separately, then merged later into the service-cost summary
...
Correct format:
```env
OPENROUTER_MODEL=anthropic/claude-3.5-sonnet  âœ…
OPENROUTER_MODEL=claude-3.5-sonnet            âŒ
```
...
**OpenRouter Integration:**
- âœ… Fully compatible with Anthropic SDK
- âœ… No code changes required
- âœ… All features supported
- âœ… Easy switching between providers
- âœ… Cost-effective alternative
- âœ… Access to 400+ models

**When to use OpenRouter:**
- ðŸ’° Want to save costs
- ðŸŒ Need access to multiple models
- ðŸ”„ Want automatic fallbacks
- ðŸ†“ Want to use free tier models

**When to use Anthropic Direct:**
- âš¡ Need lowest latency
- ðŸ”’ Require direct SLA with Anthropic
- ðŸŽ¯ Only need Claude models
