...
- âœ… **Real-time Monitoring** - Polls for new news every 5 minutes (configurable)
- âœ… **AI Review** - Automatic sentiment analysis and trading recommendations
- âœ… **Regex Keywords** - Monitor multiple symbols with `BTC|ETH|SOL`
- âœ… **WebSocket Broadcasting** - Real-time alerts to all subscribers
- âœ… **Sentiment Analysis** - POSITIVE/NEGATIVE/NEUTRAL classification
- âœ… **Impact Assessment** - HIGH/MEDIUM/LOW impact levels
- âœ… **Trading Signals** - BUY/SELL/HOLD/WAIT recommendations
- âœ… **Statistics Tracking** - Monitor performance and news counts
...
The news monitor now exposes three different time signals so the client can decide whether a headline is old, newly discovered, or simply part of the latest backend payload.

| Field | Meaning |
| --- | --- |
| `date` | original publish time from the source when available |
| `detected_at` | when Rabit first saw the article |
| `timestamp` | when the current WebSocket message or snapshot was emitted |
| `freshness_seconds` | seconds since Rabit first saw the article |
| `is_new` | backend helper that marks recently detected headlines |

For UI freshness, `detected_at` is usually the right field to sort by. `is_new` is the convenient badge field, while `freshness_seconds` is the raw number if the client wants more control.

The `is_new` helper is controlled by `NEWS_IS_NEW_WINDOW_SECONDS` and defaults to 24 hours.
...
NEWS_IS_NEW_WINDOW_SECONDS=86400
```
        ...
        for news in message['news']:
            ...
            if review.get('impact') == 'HIGH':
                ...
                if review.get('recommendation') == 'SELL':
                    print("   ðŸ”´ SELL SIGNAL - Consider closing positions")
                elif review.get('recommendation') == 'BUY':
                    print("   ðŸŸ¢ BUY SIGNAL - Consider opening positions")
